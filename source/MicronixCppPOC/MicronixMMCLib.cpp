...
bool openComPortMMC(std::string COMPort) {
    int returnVal;

    std::string preCOMAddOn = "\\\\.\\";
    COMPort = preCOMAddOn + COMPort;
    returnVal = serial.openDevice((COMPort).c_str(), 38400);
    if (returnVal == -1) {
...
string pollValues(string axis, string sendCommand)

{

    string terminator = "\r";
    string queryReturn = "\r", serialResponse = "";
    sendCommand = axis + sendCommand + "?" + terminator;
    serial.writeString((sendCommand).c_str());

    Sleep(20);
    while (serial.available() != 0) {
        serial.readString(&queryReturn[0]);
        if (queryReturn != "#") {
            serialResponse = serialResponse + queryReturn;
        }
    }
    return serialResponse;
}

bool writeCommandToMMC(string axis, string sendCommand)
{
    string terminator = "\r";

    string queryReturn = "\r", serialResponse = "";
    auto sendCommand1 = axis + sendCommand;
    sendCommand = axis + sendCommand + terminator;

    bool val = serial.writeString((sendCommand).c_str());
    Sleep(20);

    if (sendCommand1.back() == '?') {
        while (serial.available() != 0) {
            serial.readString(&queryReturn[0]);
            if (queryReturn != "#") {
                serialResponse = serialResponse + queryReturn;
...
char serialib::openDevice(const char* Device, const unsigned int Bauds,
    ...
    if (hSerial == INVALID_HANDLE_VALUE) {
        if (GetLastError() == ERROR_FILE_NOT_FOUND) {
            std::cout << "ComPort not available";
            return -1; // Device not found
    ...
    // Opening successfull
    //std::cout << "Connected Sucesffully";
    return 1;
#endif
...
int serialib::readBytes(void* buffer, unsigned int maxNbBytes, unsigned int timeOut_ms, unsigned int sleepDuration_us)
{
    ...
    // Write the parameters and return -1 if an error occrurred
    if (!SetCommTimeouts(hSerial, &timeouts)) return -1;
